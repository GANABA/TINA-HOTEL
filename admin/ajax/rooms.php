...
if (isset($_POST['add_rooms'])) {
    ...
    $q1 = "INSERT INTO `rooms` (`name`, `type`, `area`, `price`, `quantity`, `adult`, `children`, `description`) VALUES (?,?,?,?,?,?,?,?)";
    $values = [$frm_data['name'], $frm_data['type'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc']];
...
if (isset($_POST['edit_rooms'])) {
    ...
    $q1 = "UPDATE `rooms` SET `name`=?,`type`=?,`area`=?,`price`=?,`quantity`=?,`adult`=?,`children`=?,`description`=? WHERE `id`=?";
    $values = [$frm_data['name'],$frm_data['type'], $frm_data['area'], $frm_data['price'], $frm_data['quantity'], $frm_data['adult'], $frm_data['children'], $frm_data['desc'], $frm_data['room_id']];
...
// choix de l'image de mise en avant d'une chambre (thumbnail image)
