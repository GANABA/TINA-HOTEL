...
<head>
    ...
    <!-- Title -->
    <title><?php echo $settings_r['site_title'] ?> - ACCUEIL</title>
    <style>
        .availability-form {
            margin-top: -50px;
            z-index: 2;
...
<body class="bg-light">
    ...
    <div class="container availability-form">
        <div class="row">
            <div class="col-lg-12 bg-white shadow p-4 rounded">
                <h5 class="mb-4">Vérifier la disponibilité des chambres</h5>
                <form action="rooms.php">
                    <div class="row align-items-end">
                        <div class="col-lg-3 mb-3">
                            <label class="form-label" style="font-weight: 500;">Entrée</label>
                            <input type="date" class="form-control shadow-none" name="checkin" required>
                        </div>
                        <div class="col-lg-3 mb-3">
                            <label class="form-label" style="font-weight: 500;">Sortie</label>
                        ...
                        <div class="col-lg-1 mb-lg-3 mt-2">
                            <button type="submit" class="btn text-white shadow-none custum-bg">Vérifier</button>
                        </div>
                    </div>
                </form>
            </div>
        ...
        <div class="row">
            ...
            while ($room_data = mysqli_fetch_assoc($room_res)) {
                ...
                while ($fea_row = mysqli_fetch_assoc($fea_q)) {
                    $features_data .= "
                        <span class='badge text-bg-light text-wrap me-1 mb-1'>
                            $fea_row[name]
                        </span>
                    ";
                }
                ...
                if(!$settings_r['shutdown']){
                    ...
                    $book_btn = "<button onclick='checkLoginToBook($login,$room_data[id])' class='btn btn-sm text-white custum-bg shadow-none'>Réserver maintenant</button>";
                }

                // affichage des etoile d'evaluation moyen pour chaque chambre
                ...
                // affichage du nombre d'invités
                $guests = "<span class='badge text-bg-light text-wrap'>
                                $room_data[adult] Adultes
                            </span>
                            <span class='badge text-bg-light text-wrap'>
                                $room_data[children] Enfants
                            </span>";

                if($room_data['type'] == "Salle"){
                    $guests = "<span class='badge text-bg-light text-wrap'>
                        $room_data[adult] Personnes
                    </span>";
                }
                ...
                echo <<<data
                    <div class="col-lg-4 col-md-6 my-3">
                        <div class="card border-0 shadow" style="max-width: 350px; margin: auto;">
                            <img src="$room_thum" class="card-img-top">
                            <div class="card-body">
                                <h5>$room_data[name]</h5>
                                <h6 class="mb-4">$room_data[price] $price</h6>
    ...
    <h2 class="mt-5 pt-4 mb-4 text-center fw-bold h-font">NOS EQUIPEMENTS</h2>
    ...
    <div class="container">
        <div class="row justify-content-evenly px-lg-0 px-md-0 px-5">
            ...
            ?>
            <div class="col-lg-12 text-center mt-5">
                <a href="facilities.php" class="btn btn-sm btn-outline-dark rounded-0 fw-bold shadow-none">Plus d'équipements >>></a>
            </div>
        </div>
    </div>
    ...
    <h2 class="mt-5 pt-4 mb-4 text-center fw-bold h-font">TEMOINGNAGES</h2>
        ...
        <div class="swiper swiper-testimonials">
            <div class="swiper-wrapper mb-5">
                <?php
                    ...
                    $review_q = "SELECT rr.*,u.name AS uname, u.profile, r.name AS rname FROM `rating_review` rr
                    ...
                    if(mysqli_num_rows($review_res) == 0){
                        echo "Auncun avis pour le moment!";
                    }else{
    ...
    <!-- Reach us -->
    <h2 class="mt-5 pt-4 mb-4 text-center fw-bold h-font">SUIVEZ-NOUS</h2>
                ...
                <form id="recovery-form">
                    <div class="modal-header">
                        <h5 class="modal-title fs-5 d-flex align-items-center">
                            <i class="bi bi-shield-lock fs-3 me-2"></i> Définir un nouveau mot de passe
                        </h5>
                    </div>
    ...
    //Reinitialisation du compte/Mot de passe oublié
    if (isset($_GET['account_recovery'])) {
        ...
        $query = select(
            "SELECT * FROM `user` WHERE `email`=? AND `token`=? AND `t_expire`=? LIMIT 1",
        ...
        recovery_form.addEventListener('submit', function(e) {
            ...
            xhr.onload = function() {
                if (this.responseText == 'failed') {
                    alert('error', "Echec de réinitialisation du compte!");
                } else {
                    alert('success', "Réinitialisation du compte réussi!");
                    recovery_form.reset();
                }
            }
            xhr.send(data);
